...
export function generateUnifiedSetupScript(config = {}, plans = []) {
  ...
  const Q  = '\\"'
  const NL = '\n'
  const fetchUrl    = supabaseUrl + '/functions/v1/mikrotik-trigger/' + id
...
/system scheduler add name=billing-trigger-scheduler interval=5s on-event=":do {/tool fetch url=\\\"${supabaseUrl}/functions/v1/mikrotik-trigger/${id}\\\" check-certificate=no http-method=get http-header-field=\\\"Authorization: Bearer ${supabaseAnonKey},apikey: ${supabaseAnonKey}\\\" keep-result=no} on-error={:log error \\\"Heartbeat failed\\\"}"
